...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    fetchStats();
    const interval = setInterval(fetchStats, 30000); // Refresh tiap 30s
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased flex flex-col">
      {/* Header */}
      <header className="h-16 border-b border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/60 backdrop-blur-md px-4 md:px-8 flex items-center justify-between z-20 shrink-0">
        ...
        <Link
          href="/"
          className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 px-3.5 py-2 rounded-xl text-xs md:text-sm font-semibold border border-slate-200 dark:border-slate-800 transition"
        >
          ...
          <div className="bg-white dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {Object.entries(stats.perJoki)
                .sort(([, a], [, b]) => b - a)
                .map(([joki, amount]) => {
                  const total = Object.values(stats.perJoki).reduce((a, b) => a + b, 0);
