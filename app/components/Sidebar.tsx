...
export default function Sidebar({ sidebarOpen, setSidebarOpen }: Props) {
  ...
  return (
    <>
      ...
      {sidebarOpen && (
        <div
  className="fixed inset-0 bg-slate-950/40 dark:bg-slate-950/70 z-40 md:hidden"
  onClick={() => setSidebarOpen(false)}
/>
      )}

      <aside
  className={`fixed inset-y-0 left-0 z-50 w-72 h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800/80 flex flex-col justify-between p-5 transform-gpu transition-transform duration-300 ease-out ${
